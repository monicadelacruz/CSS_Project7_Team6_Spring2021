...
What is advantageous about Regression Discontinuity in terms of selection bias is that the outcome of interest is observed given a specified cutoff, thus it follows logically that those above and beyond the cutoff are not dissimilar systematically. It is quite unlikely, for example, that those above and beyond an SAT threshold for a scholarship are systematically different from each other. The main issue that RD suffers from is external validity while being close to the ideal conditions of an experiment. Thus, it's hard to generalize from an RD design but is advantageous in terms of internal validity. 
